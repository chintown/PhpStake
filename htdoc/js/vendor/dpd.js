(function (undefined) {
  ...
  var root = 'http://' + WEB_HOST; // chintown window.location.origin;
  ...
  var DEBUG = (DEV_MODE) ? '?debug=1' : ''; // chintown
  ...
  var baseMethods = {
    ...
    , del: function(options, fn) {
      ...
      return _dpd.ajax(root + joinPath(BASE_URL, options.path, query), { // chintown
          method: "DELETE"
        , query: ''
        , success: returnSuccess(fn)
        , error: returnError(fn)
  ...
  function parsePostSignature(args) {
    ...
    if (args[i] !== consoleLog && typeof args[i] === 'object') {
      ////settings.query = settings.body; // chintown
      settings.path = joinPath(settings.path, JSON.stringify(settings.body)); // chintown
      settings.body = args[i];
      i++;
    }
